...
//Day-Ahead Total Load Forecast Endpoints
router.get(`${baseURL}`+ '/DayAheadTotalLoadForecast/:AreaName/:Resolution/date/:date', authenticate, (req, res) => {
  var format = "json" ;
  ...
  var Day = date[2];
    ...
    DayAheadTotalLoadForecast.find({
      AreaName: AreaName,
      Year: Year,
      Month: Month,
      Day: Day,
          ...
          AreaTypeCode.findOne({Id: elem.AreaTypeCodeId}).then((AreaTypeCodeDoc) => {
            MapCode.findOne({Id: elem.MapCodeId}).then((MapCodeDoc) => {
                  return_object.push({
                    ...
                    DayAheadTotalLoadForecastValue: elem.TotalLoadValue,
...
router.get(`${baseURL}`+ '/DayAheadTotalLoadForecast/:AreaName/:Resolution/month/:date', authenticate, (req, res) => {
  var format = "json" ;
  if(req.query.format === "csv")
    format = "csv" ;
  else if(!req.query.format)
          ...
          MapCode.findOne({Id: elem._id.MapCodeId}).then((MapCodeDoc) => {
              return_object.push({
                ...
                Dataset: "DayAheadTotalLoadForecast",
                  ...
                  Month: Month,
                  Day: elem._id.Day,
                  DayAheadTotalLoadForecastByDayValue: elem.DayAheadTotalLoadForecastByDayValue
              });
              if(array.length == return_object.length){
...
router.get(`${baseURL}`+ '/DayAheadTotalLoadForecast/:AreaName/:Resolution/year/:date', authenticate, (req, res) => {
  var format = "json" ;
  if(req.query.format === "csv")
    format = "csv" ;
  else if(!req.query.format)
          ...
          MapCode.findOne({Id: docs[0]._id.MapCodeId}).then((MapCodeDoc) => {
              return_object.push({
                ...
                Dataset: "DayAheadTotalLoadForecast",
                  ...
                  DayAheadTotalLoadForecastByMonthValue: elem.DayAheadTotalLoadForecastByMonthValue
              });
              if(array.length == return_object.length){
